...
export default function DeliveryLinkPage() {
  ...
  const { data: subscriptions } = useCollection<any>(subscriptionsQuery);
  ...
  const handleSaveSpecific = () => {
    ...
    const currentMap = settings.customDeliveryLinkMessages || {};
    const updatedMap = {
      ...currentMap,
      [selectedSub]: specificMessage
    };

    setDocumentNonBlocking(settingsDocRef, { customDeliveryLinkMessages: updatedMap }, { merge: true });
    toast({
      title: 'Entrega específica de link salva!',
      description: `Mensagem personalizada de link definida para o produto "${selectedSub}".`
    });
  ...
  const handleRemoveSpecific = (subName: string) => {
    ...
    setDocumentNonBlocking(settingsDocRef, { customDeliveryLinkMessages: updatedMap }, { merge: true });
    toast({
      title: 'Entrega específica de link removida!',
      description: `Mensagem personalizada de link removida para "${subName}". Voltará a usar a mensagem de link geral.`
    });
  };
        ...
        <Card className="mt-6">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end rounded-md border p-4 bg-muted/20">
              <div className="space-y-2">
                ...
                <Select value={selectedSub} onValueChange={(val) => {
                  setSelectedSub(val);
                  setSpecificMessage(settings?.customDeliveryLinkMessages?.[val] || '');
                }}>
                  <SelectTrigger>
            ...
            <div className="space-y-4">
              ...
              {(!settings?.customDeliveryLinkMessages || Object.keys(settings.customDeliveryLinkMessages).length === 0) ? (
                <p className="text-sm text-muted-foreground italic">Nenhuma entrega específica configurada. Todos os planos usam a mensagem de link geral.</p>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {Object.entries(settings.customDeliveryLinkMessages).map(([subName, msg]) => (
